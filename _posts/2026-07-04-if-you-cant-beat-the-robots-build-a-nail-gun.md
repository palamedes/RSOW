...
Everywhere you look, somebody is promising that AI is going to save the world, end the world, cure cancer, write the great American novel, design your house, diagnose your rash, replace your entire team, or become the weird tireless coworker who never sleeps and answers every question with "Certainly!"

It's a lot. It's loud. And most of it is nonsense.
...
So let's do what the AC-blaming, finger-wagging, vibes-first crowd never does: let's actually look at the thing.
...
Not a replacement for the developer. A force multiplier for the developer who's willing to stop pretending the hammer is sacred.
...
Drag it into **Planning**, and the card changes character. Now an assistant reads your card *and your actual code* and starts asking the questions that turn a vague wish into a real spec. "Fix the login thing" becomes "when a user gets redirected after login, send them back to where they were trying to go — unless that URL is unsafe, in which case fall back to the dashboard." That precision matters, because AI is dramatically better when the target is clear, and most of the value is in forcing the target to *get* clear before a single line is written.

Drag it into **In Progress**, and the card changes character *again.* It's not a conversation anymore. It becomes an active worker. The card gets its own agent, its own run, its own branch, its own live timeline. The agent studies the repo, proposes a plan, and waits. You approve the plan with one click — or you don't. Then it works, and you watch it work, and it stops and asks you when it hits a real decision instead of guessing and hoping.

Drag it into **Review**, and now it's on you. You read the final report and the pull request. Something wrong? Say so, in plain English, right there in the card's conversation, and it goes back and revises. Looks good? Move it forward.
